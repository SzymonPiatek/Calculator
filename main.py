...
def create_window(theme):
    ...
    layout = [
        ...
        [sg.Button('Clear', size = size1), sg.Button('Enter', size = size1), sg.Button('(', size = size1), sg.Button(')', size = size1)],
        [sg.Button(7, size = size1), sg.Button(8, size = size1), sg.Button(9, size = size1), sg.Button('*', size = size1)],
        [sg.Button(4, size = size1), sg.Button(5, size = size1), sg.Button(6, size = size1), sg.Button('/', size = size1)],
        [sg.Button(1, size = size1), sg.Button(2, size = size1), sg.Button(3, size = size1), sg.Button('-', size = size1)],
        [sg.Button(0, expand_x = True, expand_y = True), sg.Button('.', size = size1), sg.Button('+', size = size1)] 
        ]
...
while True:
    ...
    if event in brackets:
        # Nawias jako pierwszy element przy pustej liście
        if len(full_operation) == 0:
            full_operation.append(''.join(brackets[0]))
            
        # Liczba jako pierwszy element
        elif full_operation[0][0].isdigit():
            # Ostatni element to liczba lub funkcja, ale lista jest większa lub równa 2
            if len(full_operation) >= 2 and (full_operation[-1][0].isdigit() or full_operation[-1] in functions):
                full_operation.append(''.join(event))
                
        # Nawias jako pierwszy element, ale lista nie jest pusta       
        elif full_operation[0] in brackets:
            # Ostatni element to liczba lub funkcja
            if len(full_operation) >= 2 and (full_operation[-1][0].isdigit() or full_operation[-1] in functions):
                if full_operation[-1] in functions:
                    full_operation.append(''.join(brackets[0]))
                elif full_operation[-1][0].isdigit():
    ...
    if event == 'Enter':
        if ((full_operation[-1] == brackets[0]) or (full_operation[-1] in functions)):
            full_operation.pop(-1)
        else:
            pass
